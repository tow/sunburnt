...
good_option_data = {
    ...
    FacetRangeOptions:(
        ({"fields": {"int_field": {"start": 1, "end": 10, "gap": 2}}},
         {'facet.range': ['int_field'], 'f.int_field.facet.range.start': 1, 'f.int_field.facet.range.end': 10, 'f.int_field.facet.range.gap': 2}),
        ({"fields": {"float_field": {"start": 2.5, "end": 11.5, "gap": 1.5}}},
         {'facet.range': ['float_field'], 'f.float_field.facet.range.start': 2.5, 'f.float_field.facet.range.end': 11.5, 'f.float_field.facet.range.gap': 1.5}),
        ({"fields": {"date_field": {"start": solr_date(datetime.datetime(2000,1,1)), "end": solr_date(datetime.datetime(2010,12,1)), "gap": "+1YEAR"}}},
         {'facet.range': ['date_field'], 'f.date_field.facet.range.start': u"2000-01-01T00:00:00Z", 'f.date_field.facet.range.end': u"2010-12-01T00:00:00Z", 'f.date_field.facet.range.gap': '+1YEAR'}),
        ),
    SortOptions:(
        ({"field":"int_field"},
         {"sort":"int_field asc"}),
...
bad_option_data = {
    ...
    FacetRangeOptions:(
        {"fields": {"int_field": {"start": "foo"}}}, # string is not a valid type for range facet endpoint
        {"fields": {"date_field": {"start": datetime.datetime(2010,1,1)}}}, # datetime is not a valid type for range facet endpoint
        {"fields": {"date_field": {"gap": "+1EON"}}}, # if the gap is a string, it must meet solr syntax
        ),
    SortOptions:(
